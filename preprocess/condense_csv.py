...
def compress_X(df):
    ...
    # Compress date time
    print("\ndate time size before:",
          df["date_recorded"].memory_usage(deep=True) * 1e-6)
    df["date_recorded"] = pd.to_datetime(df["date_recorded"])
    print("date time size after: ",
          df["date_recorded"].memory_usage(deep=True) * 1e-6, '\n')
...
# Reference for loading and compressing data
# train_X = pd.read_csv('../data/train.csv')
# train_y = pd.read_csv('../data/train_labels.csv')
# train_X = compress_X(train_X)
# train_y = compress_labels(train_y)